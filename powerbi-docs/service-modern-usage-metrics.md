...
- 您的 Power BI 系統管理員必須已為內容建立者啟用使用計量。 您的 Power BI 系統管理員可能也已啟用在使用計量中收集個別使用者資料。 請參閱如何[在管理入口網站中啟用這些選項](service-admin-portal.md#control-usage-metrics)。
...
使用計量報表是 Power BI 或 Office 365 系統管理員可以開啟或關閉的功能。 系統管理員可以更精細地控制哪些使用者可以存取使用計量；針對組織中的所有使用者，它們預設為「開啟」。 請參閱管理入口網站文章中的[控制使用計量](service-admin-portal.md#control-usage-metrics)來取得這些設定的詳細資料。
