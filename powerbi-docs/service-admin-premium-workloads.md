...
使用此設定可防止報表建立者發佈可能會對容量造成負面影響的大型資料集。 請注意，除非您將資料集載入記憶體中，否則 Power BI 無法判斷實際的記憶體內部大小。 離線大小較小的資料集，可能會比具有較大離線大小的資料集具有更大記憶體使用量。

如果您現有資料集大於您為此設定所指定的大小，則當使用者嘗試存取資料集時，會無法載入資料集。
...
此設定適用於資料重新整理和報表轉譯。 除非停用查詢重新整理，否則資料重新整理會從資料來源和查詢重新整理執行資料的重新整理。 如果未停用查詢重新整理，則此記憶體限制也適用於這些查詢。 任何失敗的查詢，都會將排程的重新整理狀態回報為失敗，即使資料重新整理成功也一樣。

#### <a name="query-timeout"></a>查詢逾時

使用此設定以更有效地控制長時間執行的查詢，這可能會造成使用者載入報表的速度緩慢。 此設定適用於資料重新整理和報表轉譯。 除非停用查詢重新整理，否則資料重新整理會從資料來源和查詢重新整理執行資料的重新整理。 如果未停用查詢重新整理，則此逾時限制也適用於這些查詢。
