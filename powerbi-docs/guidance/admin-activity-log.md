...
- PowerShell 指令碼的使用者必須使用 [Connect-PowerBIServiceAccount Cmdlet](/powershell/module/microsoftpowerbimgmt.profile/connect-powerbiserviceaccount) 來登入，並在出現提示時輸入 Power BI 租用戶系統管理員認證。 需要租用戶系統管理員權限才能使用活動記錄 API。

> [!IMPORTANT]
> 沒有租用戶系統管理員權限的使用者將無法在範例指令碼中執行程式碼片段。
