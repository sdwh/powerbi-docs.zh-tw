...
2. 電子郵件已收在 [訂閱]  方塊中。 您可在訂閱中新增其他電子郵件地址，但僅限於同一個網域。 若報表或儀表板託管於 [Premium 容量](/service-premium-what-is)中，您就可以使用個人電子郵件地址與群組別名訂閱，無論其是否位在網域中。 若報表或儀表板不是託管於 Premium 容量中，則您可使用其他人的名義來訂閱，但這些人也必須具有 Power BI Pro 授權。 如需詳細資料，請參閱下方的[考量與疑難排解](#considerations-and-troubleshooting)。
...
- 為避免訂閱電子郵件傳送到垃圾郵件資料夾，請將 Power BI 電子郵件別名 ([no-reply-powerbi@microsoft.com](mailto:no-reply-powerbi@microsoft.com)) 新增至連絡人。 如果您使用的是 Microsoft Outlook，請在別名上按一下滑鼠右鍵，然後選取 [新增至 Outlook 連絡人]  。
