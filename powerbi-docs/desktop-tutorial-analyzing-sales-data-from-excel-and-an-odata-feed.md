...
**Power Query 編輯器**中會開啟資料表預覽，您可以在其中套用各種轉換來清除資料。
...
1. 在 **Power Query 編輯器**中，選取 **ProductID**、**ProductName**、**QuantityPerUnit** 和 **UnitsInStock** 資料行。 您可以使用 **Ctrl**+**按一下**來選取多個資料行，或使用 **Shift**+**按一下**來選取相鄰的資料行。
   
2. 以滑鼠右鍵按一下任何選取的標題。 從下拉式清單選取 [移除其他資料行]  。 
...
當連接的資料來源擁有多個資料表時 (例如關聯式資料庫或 Northwind OData 摘要)，您可以使用資料表參考來建置查詢。 **訂單**資料表包含數個關聯資料表的參考。 您可以使用**展開**作業，將 **ProductID**、**UnitPrice** 和 **Quantity** 資料行，從關聯的 **Order_Details** 資料表新增至主題 (**Orders**) 資料表。 
...
新 **LineTotal** 資料行的資料類型為 [任何]  ，但具有貨幣值。 若要指派資料類型，請以滑鼠右鍵按一下 **LineTotal** 資料行標題，從下拉式清單選取 [變更類型]  ，然後選取 [固定的小數位數]  。 
...
1. 按兩下或點選並按住每個資料行標題，或是以滑鼠右鍵按一下資料行標題，然後從下拉式清單選取 [重新命名]  。 
...
>[!TIP]
>套用的步驟本質上是以 **Power Query 語言** (亦稱為 [**M** 語言](https://docs.microsoft.com/powerquery-m/power-query-m-reference)) 所撰寫的公式。 若要查看和編輯公式，請選取功能區 [首頁] 索引標籤之 [查詢]  群組中的 [進階編輯器]  。 

## <a name="import-the-transformed-queries"></a>匯入轉換的查詢

當您滿意轉換的資料並準備將它匯入 Power BI Desktop 報表檢視時，請在 [首頁]  功能區索引標籤的 [關閉]  群組中，選取 [關閉並套用]   > [關閉並套用]  。 
...
Power BI Desktop 不需要您結合查詢來建立報表。 不過，您可以根據通用欄位，利用資料集之間關聯性來擴充及豐富您的報表。 Power BI Desktop 會自動偵測關聯性，或您可以在 Power BI Desktop [管理關聯]  對話方塊建立它們。 如需詳細資訊，請參閱[在 Power BI Desktop 中建立和管理關聯性](desktop-create-and-manage-relationships.md)。

共用的 **ProductID** 欄位會建立此教學課程的 Orders 與 Products 資料集之間的關聯性。 

1. 在 Power BI Desktop 報表檢視中，從 [首頁]  功能區索引標籤的 [關聯性]  區域，選取 [管理關聯性]  。
   
   ![管理關聯性功能區](media/desktop-tutorial-analyzing-sales-data-from-excel-and-an-odata-feed/t_excelodata_5.png)
   
2. 在 [管理關聯性]  對話方塊中，您可以看到 Power BI Desktop 已偵測到並列出 Products 和 Orders 資料表之間的作用中關聯性。 若要檢視關聯性，請選取 [編輯]  。 
...
3. Power BI Desktop 已正確地自動偵測到關聯性，因此您可以選取 [取消]  ，然後選取 [關閉]  。
...
您可以在 Power BI Desktop 檢閱檢視中建立各種不同的視覺效果，以便深入探索資料。 報表可以有多個頁面，且每一頁可以有多種視覺效果。 您和其他人可以與視覺效果互動，以協助分析及了解資料。 如需詳細資訊，請參閱[在 Power BI 服務中與編輯檢視的報表互動](service-interact-with-a-report-in-editing-view.md)。
...
2. 若要顯示每個訂購產品的數量，請從 [欄位]  窗格的 [Products]  選取 [ProductName]  ，或將它拖曳至圖表。 
   
3. 若要按照訂購量高低順序排列產品，請選取視覺效果右上方的 [更多選項]  省略符號 ( **...** )，然後選取 [依數量排序]  。
...
2. 選取堆疊圖表，然後從 [Orders]  選取 [OrderDate]  ，或將它拖曳至圖表。 圖表現在會顯示針對每個訂單日期的產品線總數。 
   
3. 拖曳每個角落來調整視覺效果大小，並查看更多資料。 
...
2. 若要讓資料點大小反映每個國家/地區的訂單數量，請將 **LineTotal** 欄位拖曳至地圖。 您也可以將它拖曳至 [視覺效果]  窗格 [大小]  下的 [將資料欄位拖曳到此處]  。 現在在地圖上圓圈的大小會反映每個國家/地區的訂單金額。 
...
在 Power BI Desktop 中，您可以與交互醒目提示並彼此篩選的視覺效果互動，以發掘進一步的趨勢。 如需詳細資訊，請參閱 [Power BI 報表中的篩選和醒目提示](power-bi-reports-filters-and-highlighting.md)。 
