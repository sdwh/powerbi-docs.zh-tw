...
您需要必要權限，才能建立排程的重新整理計畫。 權限是在報表伺服器的角色定義中定義的。 如需詳細資訊，請參閱 SQL Server Reporting Services 文件中的[角色定義 - 預先定義的角色](https://docs.microsoft.com/sql/reporting-services/security/role-definitions-predefined-roles)。
