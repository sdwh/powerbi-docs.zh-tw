...
> [!IMPORTANT]
> 如果貴組織有多個電子郵件網域，而您希望所有的電子郵件地址擴充功能都在相同的租用戶中，請先將所有電子郵件地址網域新增至 Azure Active Directory 租用戶中，再讓使用者註冊。 使用者建立後，不會自動在租用戶之間移動。 如需此程序的詳細資訊，請參閱此文章稍後的[如果有多個網域，我可以控制使用者新增的 Office 365 租用戶嗎？](#if-i-have-multiple-domains-can-i-control-the-office-365-tenant-that-users-are-added-to)，以及[將網域新增至 Office 365](/office365/admin/setup/add-domain/)。
...
若要讓現有的使用者註冊 Power BI，請執行上述問題所列的命令，但在最後一個步驟中傳遞 true，而不傳遞 false。
...
1. 將 [Power BI (免費)] 或 [Power BI Pro] 設定為 [關閉]，視其帳戶所套用的授權而定。
...
如需 Power BI SLA (服務等級協定) 的相關資訊，請參閱 Microsoft Licensing 網站＜授權＞一節中的[授權條款與文件](http://www.microsoftvolumelicensing.com/DocumentSearch.aspx?Mode=3&DocumentTypeId=37) \(英文\) 文章。

### <a name="how-does-power-bi-handle-high-availability-and-failover"></a>Power BI 如何處理高可用性和容錯移轉？

如需高可用性和容錯移轉的資訊，請參閱 [Power BI 高可用性、容錯移轉和災害復原常見問題集](service-admin-failover.md)。
