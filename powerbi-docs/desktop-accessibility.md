...
當您在視覺效果之間瀏覽時，可以按 **Alt+Shift+F10** 將焦點移至視覺效果標頭，內含各種選項，包括排序和匯出圖表後的資料，以及焦點模式。 
...
您可以按 TAB 鍵，在交叉分析篩選器中的控制項之間移動。 在橡皮擦上按 TAB 鍵會移至下拉式按鈕。 再按一次 TAB 鍵則會移到第一個交叉分析篩選器值 (如果交叉分析篩選器有多個值時，例如一個範圍)。 
...
**Power BI Desktop** 會自動偵測 Windows 中所使用的高對比佈景主題，並將那些設定套用至您的報表。 那些高對比色彩會隨著報表一起發佈至 Power BI 服務或其他位置。
...
| 若要這樣做           | 按                |
| :------------------- | :------------------- |
| 將焦點向上 / 向下一個儲存格移 （跨所有區域中的所有儲存格）  | 向上箭頭 / 向下鍵 |
| 移動焦點向左/右一個資料格 （跨所有區域中的所有儲存格）  | 向的左鍵 / 向右鍵 |
