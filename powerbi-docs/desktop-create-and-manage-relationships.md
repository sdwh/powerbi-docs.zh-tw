...
根據預設，Power BI Desktop 會自動為您的新關聯性設定基數 (方向)、交叉篩選方向和使用中屬性；不過，您可以視需要變更設定。 如需詳細資訊，請參閱本文後段的＜了解其他選項＞一節。

如果針對關聯性所選取的資料表都沒有唯一值，您會看到錯誤指出「其中一個資料行必須具有唯一值」  。 關聯性中的至少一個資料表「必須」  具有不同且唯一的索引鍵值清單，這是對於所有關聯式資料庫技術而言常見的需求。 
...
如需更多詳細資訊，請參閱[部落格文章](https://blogs.technet.microsoft.com/cansql/2016/12/19/relationships-in-power-bi-fixing-one-of-the-columns-must-have-unique-values-error-message/)。
...
您可在建立或編輯關聯性時設定其他選項。 根據預設，會根據最佳猜測自動設定其他選項，而每個關聯性的最佳猜測，可能會根據資料行中的資料有所不同。
...
**多對多關聯性**：使用複合模型時，您可以在資料表間建立多對多關聯性，移除資料表中唯一值的需求。 此方法也會移除先前的因應措施，像是只為建立關聯性而導入新的資料表。 如需詳細資訊，請參閱[多對多基數關聯性](https://docs.microsoft.com/power-bi/desktop-many-to-many-relationships)。 
...
1. 按一下 [管理關聯性]  。
2. 在 [管理關聯性]  中，按一下 [新增]  開啟 [建立關聯性]  對話方塊，讓我們可以從中選取資料表、資料行，以及關聯性所需的其他所有設定。
...
當我們依 Priority (優先順序) 加總時數時，Power BI Desktop 會尋找 CompanyProject 查閱資料表中唯一色彩值的每個執行個體，然後在 CompanyProject 資料表中尋找這些值的每個執行個體，並計算每個唯一值的總和。

這很簡單，只要有了 [自動偵測]，您甚至可能不必進行這麼多動作。

## <a name="understanding-additional-options"></a>了解其他選項
透過 [自動偵測] 或以手動方式建立關聯性後，Power BI Desktop 會根據您資料表中的資料，自動設定其他選項。 您可在 [建立/編輯關聯性] 對話方塊的底部設定這些其他關連性屬性。
...
您可以管理 Power BI 在您報表和模型中，處理及自動調整關聯性的方式。 若要指定 Power BI 處理關聯性選項，請在 Power BI Desktop 中選取 [檔案 > 選項與設定 > 選項]  ，然後選取左側窗格中的 [資料載入]  。 您接著會看到 [關聯性]  的選項。

 ![關聯性選項](media/desktop-create-and-manage-relationships/relationships-options-01.png)

您可以選取和啟用三個選項。 

第一個選項是 [從資料來源匯入關聯性]  ，根據預設會選取此選項。 選取時，Power BI 會檢查您資料來源中定義的關聯性，例如您資料倉儲中的外部索引鍵 / 主索引鍵關聯性。 若存在這類關聯性，他們會在您一開始載入資料時鏡映到 Power BI 資料模型。 此選項可讓您快速開始使用您的模型，而不需要自行尋找或定義那些關聯性。

第二個選項是 [重新整理資料時更新或刪除關聯性]  ，根據預設此選項是關閉的。 若選取此選項 (透過選取選項內的方塊來啟用)，Power BI 會在重新整理您的資料集時，檢查資料來源關聯性中的變更。 若那些關聯性發生變更或遭到移除，Power BI 會將那些變更鏡映到其自身的資料模型，更新或刪除他們以使其相符。

> [!WARNING]
> 若您正在使用依賴已定義關聯性的資料列層級安全性，我們不建議您選取第二個選項 [重新整理資料時更新或刪除關聯性]  。 若您 RLS 設定依賴的關聯性遭到移除，您的模型安全性可能會降低。 

第三個選項是 [載入資料後自動偵測新的關聯性]  ，此選項已在本文稍早的[在載入期間自動偵測](#autodetect-during-load)一節進行描述。 
...
篩選規格會從 CompanyProject 流向 CompanyEmployee (如下圖所示)，但不會往上流向 CompanyEmployee。 不過，如果您將交叉篩選方向設定為 [雙向]，則可運作。 [雙向] 設定可讓篩選規格向上流至 [員工]。
...
現在，如果我們在報表畫布的資料表視覺效果中，建立使用 EmployeeRole 中 Role 和 Employee 欄位及 ProjectTicket 中 Hours 欄位的報表，我們只會看到專案贊助者，因為他們是開啟專案票證的唯一人員。
