...
如果您查詢兩個或多個資料表，在此同時，將資料載入時，Power BI Desktop 會嘗試尋找並為您建立的關聯性。 系統會自動設定基數、交叉篩選方向和作用中屬性。 Power BI Desktop 會查看您要查詢之資料表中的資料行名稱，以判斷是否有任何可能的關聯性。 如果有，則會自動建立這些關聯性。 如果無法判斷 Power BI Desktop 高信心層級中，沒有相符項目，它不會自動建立關聯性。 您仍然可以使用 [管理關聯性] 對話方塊來建立或編輯關聯性。
...
2. 在 [**建立關聯性**] 對話方塊中，在第一個資料表下拉式清單中，選取資料表，然後選取您想要使用的關聯性中的資料行。
3. 在第二個資料表下拉式清單中，選取您要使用的其他資料表關聯性，然後選取您想要使用此項目，然後按一下 其他資料的行**確定**。

![](media/desktop-create-and-manage-relationships/manualrelationship2.gif)

根據預設，Power BI Desktop 會自動設定基數 （方向）、 交叉篩選方向和作用中的屬性，為您新的關聯性;不過，您可以視需要變更設定。 如需詳細資訊，請參閱本文後段的＜了解其他選項＞一節。

您會看到錯誤指出*其中一個資料行必須具有唯一值*如果沒有任何關聯性所選取的資料表具有唯一值。 關聯性中的至少一個資料表「必須」  具有不同且唯一的索引鍵值清單，這是對於所有關聯式資料庫技術而言常見的需求。 
...
如需詳細資訊，請參閱 <<c0> [ 部落格文章](https://blogs.technet.microsoft.com/cansql/2016/12/19/relationships-in-power-bi-fixing-one-of-the-columns-must-have-unique-values-error-message/)。
...
您可在建立或編輯關聯性時設定其他選項。 根據預設，其他選項會自動設定根據最佳猜測，這可能會根據資料行中的每一個關聯性不同。

## <a name="cardinality"></a>基數
**多對一 (\*: 1)** -大部分常見、 預設型別，這表示一個資料表中的資料行可以有多個執行個體值和其他相關的資料表，通常稱為查閱資料表中，只有一個執行個體的值。

**一對一 (1:1)** -一個資料表中的資料行有特定的值，只有一個執行個體，而其他相關的資料表只能有特定值。

**多對多關聯性**：使用複合模型，您可以建立資料表，這會移除資料表中的唯一值的需求之間的多對多關聯性。 此方法也會移除先前的因應措施，像是只為建立關聯性而導入新的資料表。 如需詳細資訊，請參閱[多對多基數關聯性](https://docs.microsoft.com/power-bi/desktop-many-to-many-relationships)。 

如需何時變更基數的詳細資訊，請參閱本文後段的＜了解其他選項＞一節。

## <a name="cross-filter-direction"></a>交叉篩選方向
**兩者**-最常見的是，預設的方向，這表示用於篩選之用，這兩個資料表會視為它們的單一資料表。 **兩者**適用於有一些查閱資料表周圍的單一資料表。 例如，以下是具有部門之查閱資料表的銷售實際值資料表。 這通常稱為星型結構描述組態 (一個具有多個查閱資料表的中央資料表)。不過，如果您有兩個或多個資料表也有查閱資料表 (並共用一些資料表)，則不會想要使用 [雙向] 設定。 延續上一個範例，在這種情況下，您也會有一個預算銷售資料表，來記錄每個部門的目標預算。 而部門資料表會同時連接到銷售與預算資料表。 請避免針對這種組態使用 [雙向] 設定。

**單一**-篩選已連接的資料表中的選項作總值的所在資料表。 如果您匯入 Excel 2013 或更早版本的 Power Pivot 資料模型，所有關聯性都會是單向。 
...
當您透過關聯性將兩個資料表連接在一起之後，您可以使用這兩個資料表中的資料，就像是單一資料表一樣；您不需要擔心關聯性細節，也不需要先將這些資料表簡維成單一資料表，再進行匯入。 在許多情況下，Power BI Desktop 可自動為您建立關聯性，因此可能甚至不需要您自行建立這些關聯性。 不過，如果 Power BI Desktop 無法判斷與較高程度的確定性應有兩個資料表之間的關聯性，它不會自動建立關聯性。 在此情況下，您必須建立關聯性。 

讓我們逐步快速教學課程中，以進一步顯示您在 Power BI Desktop 中的關聯性的運作方式。
...
請注意，每個資料表都有一個專案資料行。 每個項目的命名方式均略有不同，但其值使各項目看似相同。 這很重要，和我們會儘快回覆進行中。
...
1. 按一下 [管理關聯性]  。
2. 在 **管理關聯性**，按一下**新增**以開啟**建立關聯性**對話方塊中，我們可以從中選取資料表、 資料行，以及所需的任何其他設定我們關聯性。
...
為了充分揭露，您剛建立此關聯性的教訓。 您可以只按一下在 [管理關聯性] 對話方塊的 [自動偵測] 按鈕。 事實上，如果兩個資料行有相同的名稱，在載入資料時，[自動偵測] 就會事先為您完成這項作業。 但這樣做哪裡還有挑戰？
...
當我們加總時數，依優先順序時，Power BI Desktop 會尋找一個獨特的色彩值，在 [公司專案] 查閱資料表中，每個執行個體和每個執行個體的每個這些值在 [公司專案] 資料表中，然後尋找和計算每個唯一值的總和.

這很簡單，事實上，與自動偵測，您可能甚至不需要執行這麼多動作項目。

## <a name="understanding-additional-options"></a>了解其他選項
建立關聯性時，自動偵測或手動的方式，建立 Power BI Desktop 會自動設定您的資料表中的資料為基礎的其他選項。 您可在 [建立/編輯關聯性] 對話方塊的底部設定這些其他關連性屬性。
...
您可以管理如何將 Power BI，並會自動調整您的報表和模型中的關聯性。 若要指定 Power BI 如何處理關聯性的選項，請選取**檔案 > 選項及設定 > 選項**從 Power BI Desktop，然後選取**資料載入**的左窗格中。 然後，您看到的選項**關聯性**。

 ![關聯性選項](media/desktop-create-and-manage-relationships/relationships-options-01.png)

有三個選項，選取並啟用。 

第一個選項是*匯入資料來源的關聯性*，並預設會選取它。 選取時，Power BI 會檢查您的資料來源，例如外部索引鍵 / 主要中定義的關聯性的索引鍵在資料倉儲中的關聯性。 如果這類關聯性，它們會鏡像至 Power BI 資料模型時一開始將資料載入。 此選項可讓您快速開始使用您的模型，而不是您需要尋找或自行定義這些關聯性。

第二個選項*更新或刪除關聯性，重新整理資料時*，和它預設為關閉。 如果選取 （已啟用選項旁邊的方塊），會檢查 Power BI 的資料來源關聯性中的變更，但重新整理您的資料集。 如果移除或變更這些關聯性，Power BI 會反映在它自己的資料模型中，這些變更，更新或刪除它們相符。

> [!WARNING]
> 如果您使用定義的關聯性所依賴的資料列層級安全性，我們不建議選取第二個選項中，*更新或刪除關聯性，重新整理資料時*。 RLS 設定依賴移除關聯性時，如果您的模型可能會變得較不安全。 

第三個選項是*自動偵測新的關聯性資料載入後*，，用來說明這[載入期間的自動偵測](#autodetect-during-load)區段中，稍早在本文中找到。 
...
篩選規格會從流向 公司專案 （如圖所示），但它不會流至 公司員工。 不過，如果您將交叉篩選方向設定為 [雙向]，則可運作。 [雙向] 設定可讓篩選規格向上流至 [員工]。
...
現在，如果我們建立使用 報表畫布中的 角色和員工的欄位，發起人和從 ProjectTickets Hours 欄位的資料表視覺效果中的報表時，我們會看到唯一的專案贊助，因為它們是開啟專案票證。
