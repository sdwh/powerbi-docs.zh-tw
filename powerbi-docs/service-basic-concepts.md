...
-  [應用程式工作區]  用於共同作業並與同事共用內容。 它們也是您為組織建立、發佈及管理應用程式的地方。 將它們視為將構成 Power BI 應用程式之內容的暫存區域與容器。 您可以將同事新增至您的應用程式工作區，以及在儀表板、報告、活頁簿和資料集上共同作業。 所有的應用程式工作區成員都需要 Power BI Pro 授權。 應用程式取用者，可以存取的應用程式的同事不一定需要 Pro 授權。 深入了解[新的工作區](service-create-the-new-workspaces.md)。  

若要深入了解共用一般情況下，著手[共用儀表板工作方式](service-how-to-collaborate-distribute-dashboards-reports.md)。
...
資料集與「工作區」  相關聯，而單一資料集可以是許多工作區的一部分。 當您開啟工作區時，相關聯的資料集會列在 [資料集]  索引標籤之下。每個列出的資料集代表單一來源的資料，例如 OneDrive 上的 Excel 活頁簿、內部部署的 SSAS 表格式資料集，或 Salesforce 資料集。 其支援許多不同的資料來源，同時我們會不停加入新的資料來源。 請參閱清單[您可以使用 Power BI 的資料集類型](service-get-data.md)。
...
新增一個工作區成員的資料集可供使用的其他工作區成員*系統管理員*，*成員*，或*參與者*角色。
...
## <a name="reports"></a>報表
Power BI 報表是一或多個頁面的視覺效果，例如折線圖、地圖及矩形式樹狀結構圖。 Virtualization 和 **_Visual_** 都是指視覺效果。 報表中的所有視覺效果都是來自單一資料集。 您可以在 Power BI 從頭開始建立報表、 與同事與您共用的儀表板匯入它們，或當您從 Excel、 Power BI Desktop、 資料庫和 SaaS 應用程式連接到資料集時，Power BI 可以建立它們。  例如，當您連接到包含 Power View 工作表的 Excel 活頁簿時，Power BI 會根據這些工作表建立報表。 而當您連接至 SaaS 應用程式時，Power BI 則會匯入預先建立的報表。

有兩種模式可以檢視報表並與其互動：*閱讀檢視*並*編輯檢視*。 當您開啟報表時，會開啟在 閱讀檢視中。 如果您具有編輯權限，則您會看到**編輯報表**在左上角，而您可以在 編輯檢視來檢視報表。  如果報表是在工作區中所有人*系統管理員*，*成員*，或*參與者*角色可以編輯它。 他們可以存取所有瀏覽、 設計、 建置和共用功能的 編輯檢視該報表。 它們共用報表的人員可以探索並與閱讀檢視 中的報表互動。   
...
在下列範例中，已選取 「 銷售與行銷 」 應用程式工作區並按一下  索引標籤**報表**。
...
* 包含在單一工作區中。
* 可以與該工作區內的多個儀表板相關聯。 圖格釘選從一份報告可以顯示多個儀表板上。
* 可使用一個資料集的資料建立而成。 Power BI Desktop 可以將一個以上的資料來源合併成單一的資料集，在報表中，和該報表可匯入 Power BI。

  ![報表圖表](media/service-basic-concepts/drawing3new.png)

### <a name="dig-deeper"></a>深入瞭解
- [在 Power BI 服務中建立報表，藉由匯入資料集](service-report-create-new.md)
- [最佳化 Power BI 行動應用程式的報表](desktop-create-phone-report.md)
...
### <a name="dig-deeper"></a>深入瞭解
* [建立空白的儀表板，然後取得一些資料](service-dashboard-create.md)。
...
### <a name="5-dashboard-title-navigation-path-or-breadcrumbs"></a>5.**儀表板標題**（導覽路徑或階層連結）
