...
針對簡單加總量值 (例如此範例中的「總銷售額」  )，則會依據相對值 (而不是絕對值) 來進行比較。 因此 Touring Bikes 的銷售額低於所有類別目錄的整體銷售時，預設視覺效果使用雙軸以允許跨不同國家/地區，Touring bikes 的所有類別與的自行車銷售的比例之間的比較。  開關視覺效果下方的切換按鈕可讓兩個值顯示在同一個軸上，以輕鬆比較絕對值 (如下圖所示)。    
...
您可能想知道什麼*不同*表示。 例如，假設美國與加拿大之間的整體銷售額分割如下：
...
因此，針對加總量值 (例如前一個範例中的「銷售額」  )，會使用直條圖和折線圖，其中使用雙軸和適當的縮放比例，以便輕鬆比較相對值。 直條圖顯示套用篩選的值，而折線圖顯示整體值 (直條圖座標軸在左側，折線圖座標軸在右側，如往常一樣)。 使用顯示的一行*階梯狀*填滿灰色的樣式，並以虛線。 如上述範例中，如果資料行軸的最大值為 4，而列軸的最大值為 20，然後它會允許篩選和整體值之間美國和加拿大相對值的比較。 
...
查看不同的資料行及每個資料行的值之後，會選擇差異最大的一組值。 為了方便了解，這些值會接著依資料行分組輸出，值差異最大的資料行會先列出。 最多三個值會顯示每個資料行，但如果有少於三個值，有很大的影響，或如果有些值會比其他人更有影響力可能顯示較少。 
