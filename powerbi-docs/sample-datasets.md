...
這些範例均以數種格式提供：內容套件、個別 Excel 活頁簿及 .pbix 檔案。 如果您不了解這些是什麼，或如何開始使用，也不必擔心。 我們會在本文稍後詳加說明。 我們為各個範例建立了「導覽」  。 導覽是說明範例背後案例並帶您瀏覽不同案例的文章。 也許會有一個案例回答您經理的問題、另一個則會談討競爭力解析，或建立報表及儀表板以供共用、或說明業務轉換。   

在開始之前，以下是使用範例的法律規範。 之後，我們會介紹範例及示範使用方式。   
...
如上方所述，範例以數種格式提供：內容套件、Excel 活頁簿及 .pbix 檔案。 我們會從內容套件開始，描述每種格式的使用方式。
