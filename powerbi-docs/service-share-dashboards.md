...
2. 在頂端的方塊中，輸入個人、通訊群組或安全性群組的完整電子郵件地址。 您無法共用動態通訊清單。 
   ...
   >[!NOTE]
   >輸入方塊最多支援 100 個使用者或群組。 如果您需要與大量使用者共用，請考慮在工作區中建立儀表板，並[將其散發為應用程式](service-create-distribute-apps.md)。
   > 
   > 
...
4. 在 [移除存取權]  對話方塊中，決定是否要一併移除報表和資料集等相關內容的存取權。 如果在移除項目時出現警告圖示 ![Power BI 警告圖示](media/service-share-dashboards/power-bi-warning-icon.png)，建議您也移除相關的內容，因為這些內容將無法正確顯示。
...
他們無法編輯這個儀表板或報表中的任何內容。 雖然他們可以與圖表互動並變更篩選條件或交叉分析篩選器，但無法儲存變更。 

只有您的直接收件者才能看到共用儀表板或報表。 例如，如果您傳送電子郵件給 Vicki@contoso.com，只有 Vicki 看得到儀表板。 沒有人可以看到儀表板，即使有連結也一樣。 Vicki 必須使用相同的電子郵件地址才能存取它；如果有人以任何其他電子郵件地址註冊，他們將無法存取儀表板。
...
如果您[允許外部來賓使用者編輯和管理組織中的內容](service-admin-portal.md#export-and-sharing-settings)，即不適用預設的僅限使用體驗。 [深入了解](service-admin-azure-ad-b2b.md)。
...
* 共用您儀表板的每個人都可以看到此儀表板，並在[閱讀檢視](consumer/end-user-reading-view.md#reading-view)中與相關的報表互動。 他們無法建立報表，或在現有的報表中儲存變更。
* 雖然沒有人可以看到或下載資料集，但他們可以使用 [在 Excel 中進行分析] 功能直接存取資料集。 系統管理員可以限制群組中的每個人能否使用 [在 Excel 中進行分析]。 不過，對於該群組中每個人的限制適用於該群組所屬的每個工作區。
* 每個人都可以手動[重新整理資料](refresh-data.md)。
* 如果電子郵件使用 Office 365，您可以輸入與通訊群組相關聯的電子郵件地址來與通訊群組的成員共用。
* 與您共用電子郵件網域的同事，以及與不同網域但在相同租用戶中註冊的同事，可以與其他人共用儀表板。 例如，如果在相同的租用戶中註冊 contoso.com 和 contoso2.com 網域，且您的電子郵件地址是 konrads@contoso.com，則可以共用 ravali@contoso.com 和 gustav@contoso2.com，只要您授與他們共用的權限即可。
