...
Power BI 分別在不同的國家/地區雲端提供。 這些雲端提供相同的層級的安全性、 隱私權、 合規性及透明度與全球版本的 Power BI，結合服務傳遞、 資料存留處、 存取和控制當地規定的獨特模型。 因此，這套專門針對當地法規而設計的獨特模型，其使用計量無法在國家地區雲端中使用。 如需詳細資訊，請參閱[國家/地區雲端](https://powerbi.microsoft.com/clouds/)。
