...
| Power BI Desktop 可用 |否 | 有。 可在 Power BI Desktop 中建置及檢視報表 |
| 訂閱 |有。 可訂閱儀表板 |有。 可訂閱報表頁面 |
| 篩選 |否。 無法篩選或配量 |有。 有多種不同方法可篩選、反白顯示及配量 |
| 精選 |有。 可將一個儀表板設為您的「精選」  儀表板 |否 |
| 我的最愛 | 有。 可將多個儀表板設為「我的最愛」  | 有。 可將多個報表設為「我的最愛」 
| 設定警示 |有。 在特定情況下可用於儀表板磚 |否 |
| 自然語言查詢 (問與答) |有 | 是，若您具有報表和基礎資料集的編輯權限 |
| 可以看到基礎資料集的資料表和欄位 |否。 可以匯出資料，但看不到儀表板本身的資料表和欄位 |有 |
