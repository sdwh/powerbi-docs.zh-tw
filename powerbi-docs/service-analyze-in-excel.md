...
## <a name="two-ways-to-get-started"></a>只要兩個步驟就能開始使用
有兩種不同方式可在 Excel 中探索 Power BI 資料集。若您是從 Power BI 開始的，則可遵循本文件中概述的步驟。  現在也可以讓具備特定 Office SKU 的使用者，直接從 Excel 活頁簿內的取得資料體驗來存取其資料集。  使用者可瀏覽其具備存取權的資料集，查看資料集是否通過認證或提升了權限，以及是否已套用保護標籤。  若要深入了解這項體驗，請參閱 Excel 文件中的[從 PowerBI 資料集建立 PivotTable](https://support.office.com/article/31444a04-9c38-4dd7-9a45-22848c666884)。
...
* 來賓使用者無法針對傳送自另一個租用戶的資料集使用 [Analyze in Excel]。 
...
## <a name="get-started-with-analyze-in-excel-in-power-bi"></a>在 Power BI 中開始使用 [使用 Excel 分析]
