...
**_徵兆：_**</br>
非系統管理員使用者第一次嘗試登入應用程式並授與同意時，會取得下列錯誤：
...
*由系統管理員授與權限*：由系統管理員將權限授與應用程式 - 針對整個租用戶或特定使用者。
...
## <a name="troubleshooting-your-embedded-application-with-the-ierror-object"></a>使用 IError 物件為您的內嵌應用程式進行疑難排解
...
| TokenExpired | 存取權杖到期，使用新的存取權杖重新提交。 無法轉譯標題如下的報表視覺效果：<visual title> | N/A | 查詢資料已到期權杖 |
