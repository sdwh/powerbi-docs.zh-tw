...
ms.locfileid: "65607862"
---
# <a name="subscribe-to-a-report-or-dashboard-in-power-bi-service"></a>訂閱 Power BI 服務中的報表或儀表板 
讓您最重要的儀表板和報表維持最新的狀態從沒這麼容易過。 訂閱報表頁面，以及對您而言最重要的儀表板，Power BI 會傳送電子郵件收件匣的快照集。 您要告訴 Power BI 需要收到電子郵件的頻率︰每天、每週，或在資料重新整理後。 您甚至可以設定特定的時間，讓 Power BI 傳送電子郵件或立即執行。  
...
4. 您的電子郵件地址會自動新增至**Subscribe**欄位。 若要訂閱其他人在您的組織，也加入他們的電子郵件地址。 
...
8. 如果一切看起來都沒有問題，請選取 [儲存並關閉]  來儲存訂用帳戶。 您和您已訂閱的同事會收到電子郵件和儀表板或報表的排程上，您設定的快照集。 所有已將頻率設定為 [在資料重新整理後]  的訂用帳戶，只會在那一天第一次排程重新整理之後傳送一封電子郵件。
   
   ![儀表板的電子郵件快照集](media/end-user-subscribe/power-bi-subscribe-email.png)
   
    重新整理報表頁面不會重新整理資料集。 只有資料集擁有者可以手動重新整理資料集。 若要查看基礎資料集的擁有者名稱，請選取**檢視相關項目**從頂端功能表列或查閱原始的訂用帳戶電子郵件。
   
    ![相關資料集](./media/end-user-subscribe/power-bi-view-related-screen.png)


## <a name="manage-your-subscriptions"></a>管理您的訂閱
只有您可以管理您所建立的訂用帳戶。 再次選取 [訂閱]  ，然後選擇左下角的 [管理所有訂用帳戶]  (請參閱上方的螢幕擷取畫面)。 
...
* 傳送給使用者的訂閱電子郵件中，釘選磚超過 25 個，或是有四個以上釘選即時報表頁面的儀表板可能不會完整呈現。 我們建議您連絡儀表板設計工具，並要求他們以減少為小於 4 以確保電子郵件正確轉譯低於 25 和已釘選即時報表釘選的磚。  
* 針對儀表板電子郵件訂用帳戶，如果有任何圖格套用資料列層級安全性 (RLS)，則不會顯示這些圖格。  針對報表電子郵件訂用帳戶，如果資料集使用 RLS，則您無法建立訂用帳戶。
* 報表頁面訂閱會繫結至報表頁面的名稱。 如果您訂閱報表頁面上，而且它重新命名，您必須重新建立您的訂用帳戶。
