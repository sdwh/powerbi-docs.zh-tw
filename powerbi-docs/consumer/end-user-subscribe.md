...
4. 填寫電子郵件訊息詳細資料。 系統會預先填入您的電子郵件，但您也可以將其他電子郵件新增至訂閱。 只能新增處於相同網域的電子郵件地址 (如需更多詳細資料，請參閱下方的**考量與疑難排解**)。 若報表或儀表板託管於 [Premium 容量](../service-premium.md)中，您就可以使用個人電子郵件地址與群組別名來為其他人訂閱。 若報表或儀表板並為託管於 Premium 容量中，您仍可使用他人的個人電子郵件地址為其訂閱，但他們也必須具有 Power BI Pro 授權。
   ...
   ![訂閱視窗](./media/end-user-subscribe/power-bi-subscribe2.png)

5. 選取 [儲存並關閉] 來儲存訂閱。 每當任何一個基礎資料集發生變更時，這些訂閱者都會收到儀表板或報表的電子郵件與快照集。 若儀表板或報表在一天內會重新整理多次，則只會在第一次重新整理後傳送電子郵件。  
   
   ![儀表板的電子郵件快照集](./media/end-user-subscribe/power-bi-dashboard-email.jpg)
   
   > [!TIP]
   > 想要立即看到電子郵件嗎？ 您可以重新整理與儀表板或報表相關的資料集，來觸發電子郵件。 (如果您沒有編輯資料集的權限，則必須要求有這些權限的人為您執行這項操作。)若要了解有哪些正在使用的資料集，請選取**檢視相關項目**圖示 ![檢視相關項目圖示](./media/end-user-subscribe/power-bi-view-related.png) 以開啟 [相關內容]，然後選取重新整理圖示 ![重新整理圖示](./media/end-user-subscribe/power-bi-refresh.png)。 
   > 
   > 
...
只有建立訂閱的使用者能夠進行管理。  有 2 條路徑可到達管理您訂閱的畫面。  第一種方法是從 [訂閱電子郵件] 對話方塊選取 [管理所有訂閱] (請參考下方的螢幕擷取畫面與上方的步驟 4)。 第二是藉由從頂端功能表列選取 Power BI 齒輪圖示 ![齒輪圖示](./media/end-user-subscribe/power-bi-settings-icon.png)，然後選擇 [設定]。

![選取 [設定]](./media/end-user-subscribe/power-bi-subscribe-settings.png)

顯示的特定訂閱將取決於哪一個工作區目前為使用中。  若要一次查看所有工作區的所有訂閱，請務必讓 [我的工作區] 為使用中。 如需了解工作區，請參閱 [Power BI 中的工作區](end-user-create-apps.md)。
