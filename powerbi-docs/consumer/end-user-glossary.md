...
**aggregates (彙總值)**：將多個資料列值群組在一起，根據準則作為輸入，形成更具重要意義或量值的單一值。

**aggregation (彙總)**：減少基礎資料來源中的資料列，以適應模型。 其結果即是彙總值。

**alert, alerts (警示)**：根據其設定的限制，通知使用者資料變更的一種功能。 警示可在從報表視覺效果釘選的圖格上設定。 使用者會在服務及其行動裝置應用程式上接收到警示。
...
**Cortana answer page (Cortana 回應頁面)**：向客戶顯示的 Cortana 頁面名稱。
...
**dashboard (儀表板)**：一組選取的視覺效果，與狀態和度量通訊。 儀表板會整理並將資料以讓人能一目了然的方式呈現。

**data classification (資料分類)**：請參閱「分類」
...
**Embedded (內嵌)**：請參閱 Power BI Embedded。 
...
**gateways (閘道)** 或**內部部署的資料閘道**：連接到基礎資料來源的橋樑。 它可在 Power BI 服務和支援重新整理之內部部署資料來源間提供快速、安全的資料轉送。 通常由 IT 進行管理。 
...
**Mobile app (行動應用程式)**：可讓您在 iOS、Android 和 Windows 裝置上執行 Power BI 的應用程式。
...
**Notification Center (通知中心)**：服務中的位置，訊息 (例如設定特定功能的通知) 會在此位置傳遞給使用者。
...
**permissions (權限)**：使用者在 Power BI 中可執行和不可執行的動作是根據權限判定。 作為「取用者」，您不會與「設計師」、系統管理員或開發人員擁有相同的權限。

**phone report (手機報表)**：已針對在手機上檢視進行格式化的 Power BI 報表名稱。
...
**QR codes (QR 代碼)**：[Power BI 行動版] 一種可為 Power BI 服務中儀表板或圖格產生的矩陣條碼，用來識別產品。 可使用 QR 代碼讀取機掃描 QR 代碼，或是使用 iOS 或 Android 上的 Power BI 行動版應用程式，直接連結到儀表板或圖格。
...
**relative links (相對連結)**：從儀表板圖格連結到其他直接共用或透過 Power BI 應用程式散發的儀表板和報表。 這可讓支援鑽研的儀表板更為豐富。
