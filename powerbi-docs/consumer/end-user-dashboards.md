...
您在儀表板上看到的視覺效果稱為「圖格」，從報表「釘選」在儀表板上。 如果您不熟悉 Power BI，請參閱 [Power BI 基本概念](end-user-basic-concepts.md)來打好基礎。
...
儀表板不只是張漂亮的圖片，其高度互動性和高度可自訂以及圖格更新會隨著基礎資料變更。
...
* 深入了解[儀表板圖格](end-user-tiles.md)以及選取一個圖格時會發生什麼情況。
* 想要追蹤個別的儀表板圖格，並在它達到某個閾值時收到電子郵件？ [建立圖格警示](end-user-alerts.md)。
