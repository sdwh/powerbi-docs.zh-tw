---
title: 在 Power BI 中的高密度散佈圖
description: 在 Power BI 中的高密度散佈圖
...
當您啟用**High-Density 取樣**，Power BI 會實作演算法會消除重疊的點，並確保視覺效果與互動時，可以達到視覺效果上的點。 演算法也可確保在視覺效果會代表資料集裡的所有點，為選取點的意義提供內容，而不只是繪製代表性的樣本。
...
新的演算法，適用於**High-Density 取樣**散佈圖會利用方法，擷取並更有效地代表基礎資料，同時消除重疊的點。 它會從每個資料點的小半徑開始 (視覺效果上的指定點的視覺效果圓形大小)。 然後增加所有資料點的半徑。當兩個 (或更多) 資料點重疊時，單一圓形 (半徑大小增加) 會代表這些重疊的資料點。 此演算法會繼續增加資料點的半徑，直到半徑值導致有合理的資料點數目 - 3,500 - 顯示在散佈圖中。
...
有些情況下這**High-Density 取樣**無法套用至散佈圖，而原始演算法使用。 這些情況如下所示：
...
若要開啟**High-Density 取樣**、 選取 散佈圖，請前往**格式化**窗格中，展開**一般**卡片，並將該卡片底部附近滑  **High-Density 取樣**切換滑桿移至**上**。
...
* **High-Density 取樣**演算法只能搭配即時連接到 Power BI 服務為基礎的模型、 匯入的模型或 DirectQuery。
